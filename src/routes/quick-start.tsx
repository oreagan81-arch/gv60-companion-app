  ...
  head: () => ({
    meta: [
      { title: "Quick Start Checklist — Owen's GV60 Companion" },
      {
        name: "description",
        content:
          "A short guided in-car checklist for the 2023 Genesis GV60 Performance, tuned for Owen Reagan.",
      },
      { property: "og:title", content: "GV60 Quick Start Checklist" },
      {
        property: "og:description",
        content: "10-minute in-car setup for Owen's Genesis GV60 Performance.",
      },
    ],
  }),
...
function QuickStart() {
  ...
  useEffect(() => {
    try {
      setDone(JSON.parse(localStorage.getItem("qs-done") || "{}"));
    } catch {
      // Progress restore is optional when storage is unavailable.
    }
  }, []);
  useEffect(() => {
    try {
      localStorage.setItem("qs-done", JSON.stringify(done));
    } catch {
      // Progress save is optional when storage is unavailable.
    }
  }, [done]);
    ...
    <div className="container-app py-8 space-y-6 pb-16">
            ...
            {section.items.map((item) => {
              ...
              return (
                <li key={item.id}>
                  ...
                  >
                    <span
                      className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border ${isDone ? "bg-primary border-primary text-primary-foreground" : "border-muted-foreground/50"}`}
                    >
                      {isDone && <span className="text-xs">✓</span>}
                    </span>
                    <span className="flex-1">
                      <span className={`block font-medium ${isDone ? "line-through" : ""}`}>
                        {item.title}
                      </span>
      ...
      <div className="card-glass p-4 text-sm text-muted-foreground">
        ...
        <Link to="/settings" className="text-primary">
          Open Owen's Recommended Settings
        </Link>{" "}
        or the{" "}
